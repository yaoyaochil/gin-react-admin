...
func (jwtService *JwtService) JwtInBlacklist(jwtList system.JwtBlacklist) (err error) {
	err = global.GraDBs["system"].Create(&jwtList).Error
	if err != nil {
		return
	}
	global.GraCache.SetDefault(jwtList.Jwt, struct{}{})
	return
}
...
func (jwtService *JwtService) IsBlacklist(jwt string) bool {
	_, ok := global.GraDBs["system"].Get(jwt)
	return ok
}
...
func LoadAll() {
	var data []string
	err := global.GraDBs["system"].Model(&system.JwtBlacklist{}).Select("jwt").Find(&data).Error
	if err != nil {
		global.GraLog.Error("加载数据库jwt黑名单失败!", zap.Error(err))
		return
	}
	for i := 0; i < len(data); i++ {
		global.GraCache.SetDefault(data[i], struct{}{})
